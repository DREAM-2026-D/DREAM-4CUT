...
| id | 이름 | | id | 이름 |
|---|---|---|---|---|
| `common` | 공통 | | `saengmyeong` | 생명대 |
| `inmun` | 인문대 | | `uikwa` | 의과대 |
| `sahoe` | 사회대 | | `ganho` | 간호대 |
| `jayeon` | 자연대 | | `yakhak` | 약학대 |
| `gyeongsang` | 경상대 | | `suui` | 수의대 |
| `gongkwa` | 공과대 | | `yedi` | 예디대 |
| `sabeom` | 사범대 | | `gyoyuk` | 교육대 |
| `haeyang` | 해양대 | | `jayu` | 자유전공 |
| | | | `daehak` | 대학원 |
...
2. **사진 칸 위치를 공통 프레임과 똑같이** 두세요. 디자인만 바꾸고
   흰 칸의 좌표는 건드리지 마세요.
...
3. **JPG 로 저장** (품질 88 정도). 한 장에 180KB 안팎이면 적당합니다.
   34장이면 6MB 정도인데, 프레임은 고를 때만 불러오므로 문제없습니다.

가장 쉬운 방법은 `common-4cut.jpg` / `common-3cut.jpg` 를 열어
...
버튼은 **점선 동그라미**로 흐리게 표시되고, 누르면 공통 프레임이 대신 나옵니다.
